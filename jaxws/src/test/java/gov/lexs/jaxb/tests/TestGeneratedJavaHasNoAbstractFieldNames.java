 ...
 * @date Dec 1, 2010
 */
public class TestGeneratedJavaHasNoAbstractFieldNames extends AbstractJavaGeneratedFileTest {

    protected List<FieldInfo> getAbstractFields(File javaFile) throws Exception {
        List<FieldInfo> abstractFields = new ArrayList<FieldInfo>();
            ...
            for( TypeDeclaration typeDecl : cu.getTypes() ){
                for( BodyDeclaration bodyDecl : typeDecl.getMembers() ){
                    if( bodyDecl instanceof FieldDeclaration ){
                        ...
                        if( fieldInfo.getFieldName().endsWith("Abstract") ){
                            abstractFields.add( fieldInfo );
                        }
                    }
                }
            }
    ...
    public void testNoRestPropertiesFound() throws Exception {
        ...
        int count = 0;
        List<FieldInfo> allAbstractFields = new ArrayList<FieldInfo>();
        for( File file : javaFiles ){
            if( (count%100) == 0 ){
                logger.debug("  Position {} of {}...", count, javaFiles.size());
            }
            allAbstractFields.addAll(this.getAbstractFields(file));
            count++;
        }

        if( !allAbstractFields.isEmpty() ){
            StringBuilder buffer = new StringBuilder();
            buffer.append("Found ").append(allAbstractFields.size()).append(" abstract field declarations:\n");
            for( int i = 0; i < allAbstractFields.size(); i++ ){
                FieldInfo field = allAbstractFields.get(i);
        ...
        logger.info("Did not find any 'abstract' properties!");
